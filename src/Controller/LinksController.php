...
class LinksController extends FrontController
{
    ...
    public function go()
    {
        ...
        //$security = $this->firewall($ad_form_data, $link, $ad_form_data['ad_type']);
    ...
    protected function firewall($data, $link, $ad_type)

    {
        ...
        if ($link->user->disable_earnings) {
            ...
            $content = [
                'status' => false,
                //'message' => 'Go without Earn because User disabled earnings',
                'message' => 'User earnings are disabled',
                'url' => $link->url,
            ];
        ...
        if ($this->isRefererBlocked($referer_url)) {
            ...
            $content = [
                'status' => false,
                //'message' => 'referer',
                'message' => 'Refer doamin is blocked',
                'url' => $link->url,
            ];
    ...
    public function shorten()
    {
        ...
        if ($this->getRequest()->getData("description")) {
            ...
            $data['description'] =  json_encode($this->getRequest()->getData('description'));
        }
