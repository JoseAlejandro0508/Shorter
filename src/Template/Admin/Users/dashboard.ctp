 ...
 * @var mixed $owner_earnings
 * @var mixed $publisher_earnings
...
<div class="box box-success">
    <div class="box-header with-border">
        <i class="fa fa-fire"></i>
        <h3 class="box-title"><?= __("Top 10 Users") ?></h3>
    </div>
    <div class="box-body">
        <table class="table table-hover table-striped">
            <thead>
                ...
                <tr>
                    <td>
                        <?= $this->Html->link($user->user->username,
                            ['controller' => 'users', 'action' => 'view', $user->user->id, 'prefix' => 'admin']);
                        ?>
                    </td>
                    <td><?= $user->views ?></td>
                    <td><?= display_price_currency($user->publisher_earnings); ?></td>
                    <td><?= display_price_currency($user->referral_earn); ?></td>
                </tr>
            <?php endforeach; ?>
            <?php unset($link) ?>
        </table>
